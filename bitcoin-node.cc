...
BitcoinNode::StartApplication ()    // Called at time specified by Start
{
  ...
  m_nodeStats->CmpctReceivedBytes=0;
  m_nodeStats->BlockTxnReceivedBytes=0;
  m_nodeStats->GetBlockTxnReceivedBytes=0;
  m_nodeStats->MsgCmpctBlockReceivedBytes=0;
  m_nodeStats->SendCmpctReceivedBytes=0;
  m_nodeStats->CmpctSentBytes=0;
  m_nodeStats->BlockTxnSentBytes=0;
  m_nodeStats->GetBlockTxnSentBytes=0;
  m_nodeStats->MsgCmpctBlockSentBytes=0;
  m_nodeStats->SendCmpctSentBytes=0;
  m_nodeStats->CmpctBlockNotProcessed=0;
  m_nodeStats->NumGBTSentInCmpct=0;
  m_nodeStats->NumFullBlockInCmpct=0;


  if(m_protocolType==COMPACT_BLOCK)
    setCompact(m_maxHighBandwidthRelay);
  // ...
...
value=1;
d.AddMember("int",value,d.GetAllocator());
std::cout<<d["message"].GetInt()<<" "<<d["int"].GetInt()<<"\n";
// Create DOM by Set()
rapidjson::Pointer("/project").Set(d, "RapidJSON");
rapidjson::Pointer("/stars").Set(d, 10);
 
std::cout<<d["stars"].GetInt();
// { "project" : "RapidJSON", "stars" : 10 }
 
// Access DOM by Get(). It return nullptr if the value does not exist.
if (rapidjson::Value* stars = rapidjson::Pointer("/stars").Get(d))
        ...
        while ((pos = totalReceivedData.find(delimiter)) != std::string::npos) 
        {
          ...
          switch (d["message"].GetInt())
          {
            ...
            */
            ...
            {
              //NS_LOG_INFO ("INV");
              //std::cout<<"Node "<<GetNode()->GetId()<<" received INV from peerAddress "<<InetSocketAddress::ConvertFrom(from).GetIpv4 ()<< " at time "<<Simulator::Now().GetSeconds()<<"\n";
              int j;
              ...
              if (!requestBlocks.empty())
              {
                ...
                Ipv4Address outgoingIpv4Address = InetSocketAddress::ConvertFrom(from).GetIpv4 ();
                
                bool highbandwidth=false, lowbandwidth=false;
                std::vector<Ipv4Address>::iterator it = std::find(m_sendLowBandwidth.begin(),m_sendLowBandwidth.end(),outgoingIpv4Address);
                
                if(it!=m_sendLowBandwidth.end())
                    lowbandwidth=true;

                it = std::find(m_sendHighBandwidth.begin(),m_sendHighBandwidth.end(),outgoingIpv4Address);
                
                if(it!=m_sendHighBandwidth.end())
                    highbandwidth=true;

                if(m_protocolType==COMPACT_BLOCK && (lowbandwidth==true || highbandwidth==true))
                  {
                    SendMessage(INV, MSG_CMPCT_BLOCK, d, from);
                  }
                else      
                  {
                    SendMessage(INV, GET_DATA, d, from);
...
/*                     PrintQueueChunks();
            ...
            case GET_HEADERS:
            {
              //std::cout<<"Node "<<GetNode()->GetId()<<" received GET_HEADERS from peerAddress "<<InetSocketAddress::ConvertFrom(from).GetIpv4 ()<< " at time "<<Simulator::Now().GetSeconds()<<"\n";
              int j;
            ...
            case GET_DATA:
            {
             // std::cout<<"Node "<<GetNode()->GetId()<<" received GET_DATA from peerAddress "<<InetSocketAddress::ConvertFrom(from).GetIpv4 ()<< " at time "<<Simulator::Now().GetSeconds()<<"\n";
...
/*                 std::cout << "Node " << GetNode()->GetId() << "-" << InetSocketAddress::ConvertFrom(from).GetIpv4 () 
            ...
            case HEADERS:
            {
              //std::cout<<"Node "<<GetNode()->GetId()<<" received HEADERS from peerAddress "<<InetSocketAddress::ConvertFrom(from).GetIpv4 ()<< " at time "<<Simulator::Now().GetSeconds()<<"\n";
              NS_LOG_INFO ("HEADERS");
              ...
              {  
                ...
                {				  
                  ...
                  {
                    //std::cout<<"requesting  headers and MSG_CMPCT_BLOCK of parent block in HEADERS\n";
                    NS_LOG_INFO("HEADERS: Bitcoin node " << GetNode ()->GetId ()
                                 << " has not requested its parent block yet");
								 
                    if(m_protocolType == STANDARD_PROTOCOL || m_protocolType==COMPACT_BLOCK ||
                  ...
                  {
                    //std::cout<<"NOT requesting  headers in HEADERS\n";
                    NS_LOG_INFO("HEADERS: Bitcoin node " << GetNode ()->GetId ()
                                 << " has already requested the block");
                  }
                ...
                {
                  //std::cout<<"In HEADERS.. The block is not an orphan... Not requesting blocks\n";
                  /**
              ...
              if (!requestHeaders.empty())
              {
                ...
                if(m_protocolType!=COMPACT_BLOCK)			
                  ...
                  SendMessage(HEADERS, MSG_CMPCT_BLOCK, d, from);
              }
			  
              if (!requestBlocks.empty())
              {
                //std::cout<<"requesting BLOCK after receiving HEADERS\n";
                rapidjson::Value   value;
                rapidjson::Value   array(rapidjson::kArrayType);
                Time               timeout;
...
/*                     std::cout << "candidateChunks = ";SendMessage
                    for (auto chunk : candidateChunks)
                      std::cout << chunk << ", ";
                    std::cout << "\n"; */
...
/*                     PrintQueueChunks();
            ...
            case BLOCK:
            {
             // std::cout<<"Node "<<GetNode()->GetId()<<" received BLOCK from peerAddress "<<InetSocketAddress::ConvertFrom(from).GetIpv4 ()<< " at time "<<Simulator::Now().GetSeconds()<<"\n";
              NS_LOG_INFO ("BLOCK");
              //std::cout<<"Node "<<GetNode()->GetId()<<" received BLOCK from peerAddress "<<InetSocketAddress::ConvertFrom(from).GetIpv4 ()<< " at time "<<Simulator::Now().GetSeconds()<<"\n";
            ...
            {
              NS_LOG_INFO ("MSG_CMPCT_BLOCK");
              m_nodeStats->MsgCmpctBlockReceivedBytes+=m_bitcoinMessageHeader + m_countBytes + d["blocks"].Size()*m_inventorySizeBytes;
              //std::cout<<"Node "<<GetNode()->GetId()<<" received MSG_CMPCT_BLOCK from peerAddress "<<InetSocketAddress::ConvertFrom(from).GetIpv4 ()<< " at time "<<Simulator::Now().GetSeconds()<<"\n";
              int j;
              std::vector<Block>              requestCmpct;
              std::vector<Block>              requestBlocks;
                std::vector<Block>::iterator    block_it;
                //m_nodeStats->msgCmpctBlockreceivedBytes += m_bitcoinMessageHeader + m_countBytes + d["blocks"].Size()*m_inventorySizeBytes;
                ...
                for(j=0;j<d["blocks"].Size();j++)
                {
                  ...
                  it=find(m_queueCmpct.begin(),m_queueCmpct.end(),parsedInv);
              
                  if (m_blockchain.HasBlock(height, minerId) && it!=m_queueCmpct.end())
                  {
                    NS_LOG_INFO("GET_DATA: Bitcoin node " << GetNode ()->GetId () 
                                << " has already received the block with height = " 
                                << height << " and minerId = " << minerId);
                    Block newBlock (m_blockchain.ReturnBlock (height, minerId));
                    requestCmpct.push_back(newBlock);
                  }
                  else if(m_blockchain.HasBlock(height, minerId) && it==m_queueCmpct.end())
                  {
                ...
                if (!requestCmpct.empty())
                {
                  unsigned int totalBlockMessageSize=0;
                  rapidjson::Value value;
                  rapidjson::Value array(rapidjson::kArrayType);
                    

                  d.RemoveMember("blocks");
                  for (block_it = requestCmpct.begin(); block_it < requestCmpct.end(); block_it++) 
                  {
                    unsigned int blockSize = block_it->GetBlockSizeBytes ();

                    std::random_device gen;
                    std::poisson_distribution<> distr((m_CmpctEstimateShortIdTransactions)*(2147483647/100.0));
                    double temp=(distr(gen))/2147483647.0;
                    int    numOfTransactions = static_cast<int>((blockSize - m_blockHeadersSizeBytes)/m_averageTransactionSize);
                    
                    unsigned int numOfShortID=(temp*numOfTransactions);
                    unsigned int numOfPrefilledTransactions=numOfTransactions-numOfShortID;
                    if(numOfPrefilledTransactions*(2+m_averageTransactionSize)>10000)
                    {
                      numOfShortID+=numOfPrefilledTransactions-1;
                      numOfPrefilledTransactions=1;
                    }
                    blockSize=92+numOfShortID*6+numOfPrefilledTransactions*(2+m_averageTransactionSize);

                    rapidjson::Value blockInfo(rapidjson::kObjectType);
                    NS_LOG_INFO ("In requestCmpct " << *block_it);
                    ...
                    value=numOfShortID;
                    blockInfo.AddMember("numOfShortID", value, d.GetAllocator());

                    value=numOfPrefilledTransactions;
                    blockInfo.AddMember("numOfPrefilledTransactions", value, d.GetAllocator());

                    value=numOfShortID+numOfPrefilledTransactions;
                    blockInfo.AddMember("numOfTransactions", value, d.GetAllocator());

                    value=0;
                    blockInfo.AddMember("sendGetHeaders", value, d.GetAllocator());
                  
                    array.PushBack(blockInfo, d.GetAllocator());
                  } 
                
                  d.AddMember("cmpct", array, d.GetAllocator());
                ...
                double eventTime; 
                  ...
                  Simulator::Schedule (Seconds(eventTime), &BitcoinNode::SendCmpctBlock, this, packet, from);
                  Simulator::Schedule (Seconds(eventTime + sendTime), &BitcoinNode::RemoveSendTime, this);
            ...
            /*                 std::cout << "Node " << GetNode()->GetId() << "-" << InetSocketAddress::ConvertFrom(from).GetIpv4 () 
                  ...
                  Simulator::Schedule (Seconds(eventTime), &BitcoinNode::MCBSendBlock, this, packet, from);
                  Simulator::Schedule (Seconds(eventTime + sendTime), &BitcoinNode::RemoveSendTime, this);
            ...
            case CMPCT:
            {
              //std::cout<<"Node "<<GetNode()->GetId()<<" received CMPCT from peerAddress "<<InetSocketAddress::ConvertFrom(from).GetIpv4 ()<< " at time "<<Simulator::Now().GetSeconds()<<"\n";
              int j;
              unsigned int numOfShortID=0;
              unsigned int numOfPrefilledTransactions=0;
              for(j=0;j<d["cmpct"].Size();j++)
              {
                numOfShortID+=d["cmpct"][j]["numOfShortID"].GetInt();
                numOfPrefilledTransactions+=d["cmpct"][j]["numOfPrefilledTransactions"].GetInt();
              }
              double blockSize=92*d["cmpct"].Size()+numOfShortID*6+numOfPrefilledTransactions*(m_averageTransactionSize+2);
              m_nodeStats->CmpctReceivedBytes+=blockSize;
              ...
              if (m_receiveBlockTimes.size() == 0 || Simulator::Now ().GetSeconds() >  m_receiveBlockTimes.back())
              {
                receiveTime = blockSize / m_downloadSpeed; 
                eventTime = blockSize / minSpeed;
              }
              else
              {
                receiveTime = blockSize / m_downloadSpeed + m_receiveBlockTimes.back() - Simulator::Now ().GetSeconds(); 
                eventTime = blockSize / minSpeed + m_receiveBlockTimes.back() - Simulator::Now ().GetSeconds(); 
              }
              m_receiveBlockTimes.push_back(Simulator::Now ().GetSeconds() + receiveTime);
              Simulator::Schedule (Seconds(eventTime), &BitcoinNode::ReceivedCmpctBlockMessage, this, help, from);
              Simulator::Schedule (Seconds(receiveTime), &BitcoinNode::RemoveReceiveTime, this);
            ...
            case GET_BLOCK_TXN:
            {
              //std::cout<<"Node "<<GetNode()->GetId()<<" received GET_BLOCK_TXN from peerAddress "<<InetSocketAddress::ConvertFrom(from).GetIpv4 ()<< " at time "<<Simulator::Now().GetSeconds()<<"\n";
              int j;
              double blockSize=0;
              for(j=0;j<d["cmpct"].Size();j++)
                blockSize+=d["cmpct"][j]["size"].GetInt();

              m_nodeStats->GetBlockTxnReceivedBytes+=blockSize;
              ...
              m_receiveBlockTimes.push_back(Simulator::Now ().GetSeconds() + receiveTime);
              Simulator::Schedule (Seconds(eventTime), &BitcoinNode::ReceivedGBTMessage, this, help, from);
              Simulator::Schedule (Seconds(receiveTime), &BitcoinNode::RemoveReceiveTime, this);
            ...
            case BLOCK_TXN:
            {
              //std::cout<<"Node "<<GetNode()->GetId()<<" received BLOCK_TXN from peerAddress "<<InetSocketAddress::ConvertFrom(from).GetIpv4 ()<< " at time "<<Simulator::Now().GetSeconds()<<"\n";
              int j;
              double blockSize=0;
              for(j=0;j<d["cmpct"].Size();j++)
                blockSize+=d["cmpct"][j]["size"].GetInt();

              m_nodeStats->BlockTxnReceivedBytes+=blockSize;
              ...
              m_receiveBlockTimes.push_back(Simulator::Now ().GetSeconds() + receiveTime);
              Simulator::Schedule (Seconds(eventTime), &BitcoinNode::ReceivedBTMessage, this, help, from);
              Simulator::Schedule (Seconds(receiveTime), &BitcoinNode::RemoveReceiveTime, this);
            ...
            case SENDCMPCT:
            {
              //std::cout<<"Node "<<GetNode()->GetId()<<" received SENDCMPCT from peerAddress "<<InetSocketAddress::ConvertFrom(from).GetIpv4 ()<< " at time "<<Simulator::Now().GetSeconds()<<"\n";
              m_nodeStats->SendCmpctReceivedBytes+=9;
              if(d["bandwidth"].GetInt()==1)
                  m_sendHighBandwidth.push_back(InetSocketAddress::ConvertFrom(from).GetIpv4 ());
              else if (d["bandwidth"].GetInt()==0)
                  m_sendLowBandwidth.push_back(InetSocketAddress::ConvertFrom(from).GetIpv4 ());
              break;
            }
            default:
              NS_LOG_INFO ("Default");
...
void BitcoinNode::SendBlockTxn(std::string blockInfo, Address &from)
{
  SendMessage(GET_BLOCK_TXN, BLOCK_TXN, blockInfo, from);
}
void BitcoinNode::GBTSendBlock(std::string blockInfo, Address &from)
{

  SendMessage(GET_BLOCK_TXN, BLOCK, blockInfo, from);
}

void BitcoinNode:: setCompact(int maxHighBandwidth)
{

  m_nodeStats->SendCmpctSentBytes+=9*m_peersAddresses.size();

  int index=0,bandwidth;
  rapidjson::Document d;
  d.SetObject();
  rapidjson::Value value;
  
  value=1;
  d.AddMember("bandwidth",value,d.GetAllocator());
  value=SENDCMPCT;
  d.AddMember("message",value,d.GetAllocator());
  rapidjson::StringBuffer packetInfo;
  rapidjson::Writer<rapidjson::StringBuffer> writer(packetInfo);
  ...
  std::vector<Ipv4Address> AddressCopy=m_peersAddresses;
  while(index<maxHighBandwidth && AddressCopy.size()>0)
  {
    int random=rand()%AddressCopy.size();
    m_receiveHighBandwidth.push_back(AddressCopy[random]);
    const uint8_t delimiter[] = "#";
    m_peersSockets[AddressCopy[random]]->Send (reinterpret_cast<const uint8_t*>(packetInfo.GetString()), packetInfo.GetSize(), 0);
    m_peersSockets[AddressCopy[random]]->Send (delimiter, 1, 0);
    index++;
    AddressCopy.erase(AddressCopy.begin()+random);
  }

  /*for(it=m_peersAddresses.begin();it!=m_peersAddresses.end();it++)
  {
    if(index>=maxHighBandwidth)
      break;
    m_receiveHighBandwidth.push_back(*it);
    const uint8_t delimiter[] = "#";
    m_peersSockets[*it]->Send (reinterpret_cast<const uint8_t*>(packetInfo.GetString()), packetInfo.GetSize(), 0);
    m_peersSockets[*it]->Send (delimiter, 1, 0);
    index++;
  }*/
  d["bandwidth"].SetInt(0);
  rapidjson::StringBuffer packetInfo1;
  rapidjson::Writer<rapidjson::StringBuffer> writer1(packetInfo1);
  d.Accept(writer1);
  for(int i=0;i<AddressCopy.size();i++)
  {
    m_receiveLowBandwidth.push_back(AddressCopy[i]);
      const uint8_t delimiter[] = "#";
      m_peersSockets[AddressCopy[i]]->Send (reinterpret_cast<const uint8_t*>(packetInfo1.GetString()), packetInfo1.GetSize(), 0);
      m_peersSockets[AddressCopy[i]]->Send (delimiter, 1, 0);
  }
  /*index=0;
  for(it=m_peersAddresses.begin();it!=m_peersAddresses.end();it++)
  {
    if(index>=maxHighBandwidth)
    {
      m_receiveLowBandwidth.push_back(*it);
      const uint8_t delimiter[] = "#";
      m_peersSockets[*it]->Send (reinterpret_cast<const uint8_t*>(packetInfo1.GetString()), packetInfo1.GetSize(), 0);
      m_peersSockets[*it]->Send (delimiter, 1, 0);
    }
  index++;
  }*/
...
void 
BitcoinNode::SendCmpctBlock(std::string packetInfo, Address& from) 
{
  NS_LOG_FUNCTION (this);
  
  NS_LOG_INFO ("SendCMPCTBlock: At time " << Simulator::Now ().GetSeconds ()
  ...
  SendMessage(MSG_CMPCT_BLOCK, CMPCT, packetInfo, from);
}

void BitcoinNode::SendGetBlockTxn(std::string &msgInfo, Address &from)
{
  rapidjson::Document d;
  d.Parse(msgInfo.c_str());
  SendMessage(CMPCT, GET_BLOCK_TXN, d, from);
}

void BitcoinNode::CalculateCmpctBlock(std::string &blockHash)
{
  if(m_CmpctBufferSize==0)
    return;
  if(m_queueCmpct.size()<m_CmpctBufferSize)
    m_queueCmpct.push_back(blockHash);
  else
  {
    m_queueCmpct.erase(m_queueCmpct.begin());
    m_queueCmpct.push_back(blockHash);
  }
 
}

void BitcoinNode::MCBSendBlock(std::string &blockInfo, Address &from)
{
  SendMessage(MSG_CMPCT_BLOCK, BLOCK, blockInfo, from);
}


void BitcoinNode::ReceivedGBTMessage(std::string &blockInfo, Address &from)
{
  rapidjson::Document d;
  d.Parse(blockInfo.c_str());

  int j;
  std::vector<int> sendBlock,sendBT;
  ...
  for(j=0;j<d["cmpct"].Size();j++)
  {
    ...
    int height = d["cmpct"][j]["height"].GetInt();
    int minerId = d["cmpct"][j]["minerId"].GetInt();
    stringStream<< height<<"/"<<minerId;
    std::string blockHash = stringStream.str();

    d["cmpct"][j]["size"].SetInt(34+d["cmpct"][j]["numOfShortID"].GetInt()*m_averageTransactionSize);
    
    if(m_blockchain.HasBlock(height, minerId))
    {
      std::vector<std::string>::iterator it;
      it=find(m_queueCmpct.begin(),m_queueCmpct.end(),blockHash);
      if(it!=m_queueCmpct.end())
      { sendBT.push_back(j);

      }
      else
      { sendBlock.push_back(j);
  ...
  if(!sendBT.empty())
  {
    ...
    for(j=0;j<sendBT.size();j++)
    {
      rapidjson::Value blockInfo(rapidjson::kObjectType);

      value = d["cmpct"][sendBT[j]]["height"].GetInt();
      blockInfo.AddMember("height", value, doc.GetAllocator ());

      value = d["cmpct"][sendBT[j]]["minerId"].GetInt();
      blockInfo.AddMember("minerId", value, doc.GetAllocator ());

      value = d["cmpct"][sendBT[j]]["parentBlockMinerId"].GetInt();
      blockInfo.AddMember("parentBlockMinerId", value, doc.GetAllocator ());
      
      unsigned int swap = d["cmpct"][sendBT[j]]["numOfShortID"].GetInt()*m_averageTransactionSize + 34;
      value=swap;
      totalBlockMessageSize += value.GetInt();
      blockInfo.AddMember("size", value, doc.GetAllocator ());

      value = d["cmpct"][sendBT[j]]["timeCreated"].GetDouble();
      blockInfo.AddMember("timeCreated", value, doc.GetAllocator ());

      value = d["cmpct"][sendBT[j]]["timeReceived"].GetDouble();             
      blockInfo.AddMember("timeReceived", value, doc.GetAllocator ());

      value = d["cmpct"][sendBT[j]]["numOfShortID"].GetInt();
      blockInfo.AddMember("numOfShortID", value, doc.GetAllocator());
      
      value = d["cmpct"][sendBT[j]]["numOfPrefilledTransactions"].GetInt();
      blockInfo.AddMember("numOfPrefilledTransactions", value, doc.GetAllocator());

      value = d["cmpct"][sendBT[j]]["numOfTransactions"].GetInt();
      blockInfo.AddMember("numOfTransactions", value, doc.GetAllocator());

      array.PushBack(blockInfo, doc.GetAllocator());
    }
    doc.AddMember("cmpct",array,doc.GetAllocator());
    value=GET_BLOCK_TXN;
    doc.AddMember("message", value, doc.GetAllocator());


    double eventTime, sendTime;
    if(m_sendBlockTimes.size()==0 || Simulator::Now().GetSeconds()>m_sendBlockTimes.back())
      eventTime=0;
    else
      eventTime= m_sendBlockTimes.back() - Simulator::Now().GetSeconds();
    ...
    Simulator::Schedule(Seconds(eventTime), &BitcoinNode::SendBlockTxn, this, packet, from );
    Simulator::Schedule(Seconds(eventTime + sendTime), &BitcoinNode::RemoveSendTime, this);
  }
  if(!sendBlock.empty())
  {
    ...
    for(j=0;j<sendBlock.size();j++)
    {
      rapidjson::Value blockInfo(rapidjson::kObjectType);

      value = d["cmpct"][sendBlock[j]]["height"].GetInt();
      blockInfo.AddMember("height", value, doc.GetAllocator ());

      value = d["cmpct"][sendBlock[j]]["minerId"].GetInt();
      blockInfo.AddMember("minerId", value, doc.GetAllocator ());

      value = d["cmpct"][sendBlock[j]]["parentBlockMinerId"].GetInt();
      blockInfo.AddMember("parentBlockMinerId", value, doc.GetAllocator ());
      
      unsigned int sizeOfBlock = d["cmpct"][sendBlock[j]]["numOfTransactions"].GetInt()*m_averageTransactionSize + m_blockHeadersSizeBytes;
      value=sizeOfBlock;
      totalBlockMessageSize += value.GetInt();
      blockInfo.AddMember("size", value, doc.GetAllocator ());

      value = d["cmpct"][sendBlock[j]]["timeCreated"].GetDouble();
      blockInfo.AddMember("timeCreated", value, doc.GetAllocator ());

      value = d["cmpct"][sendBlock[j]]["timeReceived"].GetDouble();             
      blockInfo.AddMember("timeReceived", value, doc.GetAllocator ());
    ...
    Simulator::Schedule(Seconds(eventTime), &BitcoinNode::GBTSendBlock, this,  packet, from );
    Simulator::Schedule(Seconds(eventTime + sendTime), &BitcoinNode::RemoveSendTime, this);
  }

}

void BitcoinNode::ReceivedBTMessage(std::string &blockInfo, Address &from)
{
  rapidjson::Document d;
  d.Parse(blockInfo.c_str());
  int j;
  ...
  for(j=0;j<d["cmpct"].Size();j++)
  {
    ...
    {
      receivedBlocks.push_back(j);         
    }
  }
  if(!receivedBlocks.empty())
  {
    for(j=0;j<receivedBlocks.size();j++)
    {
      rapidjson::Value blockObj(rapidjson::kObjectType);
      value = d["cmpct"][receivedBlocks[j]]["height"].GetInt();
      blockObj.AddMember("height", value, doc.GetAllocator ());

      value = d["cmpct"][receivedBlocks[j]]["minerId"].GetInt();
      blockObj.AddMember("minerId", value, doc.GetAllocator ());

      value = d["cmpct"][receivedBlocks[j]]["parentBlockMinerId"].GetInt();
      blockObj.AddMember("parentBlockMinerId", value, doc.GetAllocator ());
      
      unsigned int blockSize;
      blockSize=d["cmpct"][receivedBlocks[j]]["numOfTransactions"].GetInt()*m_averageTransactionSize;
      value = blockSize;
      totalBlockMessageSize+=value.GetInt();
      blockObj.AddMember("size", value, doc.GetAllocator ());

      value = d["cmpct"][receivedBlocks[j]]["timeCreated"].GetDouble();
      blockObj.AddMember("timeCreated", value, doc.GetAllocator ());

      value = d["cmpct"][receivedBlocks[j]]["timeReceived"].GetDouble();             
      blockObj.AddMember("timeReceived", value, doc.GetAllocator ());
    
      array.PushBack(blockObj, doc.GetAllocator());
    }
    
    doc.AddMember("blocks", array, doc.GetAllocator());
  
    rapidjson::StringBuffer blockbuff;
    rapidjson::Writer<rapidjson::StringBuffer> blockWriter(blockbuff);
    doc.Accept(blockWriter);
    std::string help = blockbuff.GetString();
    ...
    if (m_receiveBlockTimes.size() == 0 || Simulator::Now ().GetSeconds() >  m_receiveBlockTimes.back())
    {
      receiveTime = totalBlockMessageSize / m_downloadSpeed; 
      eventTime = totalBlockMessageSize / minSpeed;
    }
    else
    {
      receiveTime = totalBlockMessageSize / m_downloadSpeed + m_receiveBlockTimes.back() - Simulator::Now ().GetSeconds(); 
      eventTime = totalBlockMessageSize / minSpeed + m_receiveBlockTimes.back() - Simulator::Now ().GetSeconds(); 
    }
    m_receiveBlockTimes.push_back(Simulator::Now ().GetSeconds() + receiveTime);
    Simulator::Schedule (Seconds(eventTime), &BitcoinNode::ReceivedBlockMessage, this, help, from);
    Simulator::Schedule (Seconds(receiveTime), &BitcoinNode::RemoveReceiveTime, this);
...
void
BitcoinNode::ReceivedCmpctBlockMessage(std::string &blockInfo, Address &from)
{
  NS_LOG_FUNCTION (this);
  rapidjson::Document d;
  d.Parse(blockInfo.c_str());
  
  rapidjson::Document head;
  head.SetObject();
  rapidjson::Value   valueHead;
  rapidjson::Value   arrayHead(rapidjson::kArrayType);

  NS_LOG_INFO("ReceivedCmpctBlockMessage: At time " << Simulator::Now ().GetSeconds () 
              << " Node " << GetNode()->GetId() << " received a block message " << blockInfo);

  //m_receiveBlockTimes.erase(m_receiveBlockTimes.begin());
  int j;
  std::vector<int>              requestBlocks;
  std::vector<int>              requestHeaders;
  std::vector<double>           tempValue;
  std::vector<int>              receivedBlocks;
  std::vector<double>              distributionValue;
  std::vector<Block>::iterator    block_it;
  for (j=0; j<d["cmpct"].Size(); j++)
  {  
    std::ostringstream stringStream;
    ...
    int minerId = d["cmpct"][j]["minerId"].GetInt();
    stringStream<< height<<"/"<<minerId;
    std::string parsedInv = stringStream.str();

    if(m_invTimeouts.find(parsedInv) != m_invTimeouts.end())
    {
      m_queueInv.erase(parsedInv);
      Simulator::Cancel (m_invTimeouts[parsedInv]);
      m_invTimeouts.erase(parsedInv);
    }                


    if (m_receivedCmpctBlock[parsedInv]==true || m_blockchain.HasBlock(height, minerId) || m_blockchain.IsOrphan(height, minerId) || ReceivedButNotValidated(parsedInv))
    {
      NS_LOG_INFO("CMPCT: Bitcoin node " << GetNode ()->GetId () 
                  << " has already received the block with height = " 
                  << height << " and minerId = " << minerId);         
      m_nodeStats->CmpctBlockNotProcessed+=d["cmpct"][j]["size"].GetInt();
    }
    else
    {
      
      if(d["cmpct"][j]["sendGetHeaders"].GetInt()==1)
        {
          requestHeaders.push_back(j);
          valueHead.SetString(parsedInv.c_str(), parsedInv.size(), head.GetAllocator());
          arrayHead.PushBack(valueHead, head.GetAllocator());
        }
        else
        {
          //std::cout<<"In CMPCT... not requesting headers\n";
        }
      ...
      std::random_device gen;
      std::poisson_distribution<> distr((m_CmpctEstimatedTransactionsNotPresent)*(2147483647/100.0));
      double temp=(distr(gen))/2147483647.0;
      if(temp>0.1)
      {
        int exchange=d["cmpct"][j]["numOfShortID"].GetInt()*temp;
        d["cmpct"][j]["numOfShortID"].SetInt(exchange);
        requestBlocks.push_back(j);
      }
      else
      {
        receivedBlocks.push_back(j);
    ...
    m_receivedCmpctBlock[parsedInv]=true;                 
  }

  if(!requestHeaders.empty())
  {
    head.AddMember("blocks", arrayHead, head.GetAllocator());
    valueHead=CMPCT;
    head.AddMember("message", valueHead, head.GetAllocator());
    valueHead.SetString("block");
    head.AddMember("type", valueHead, head.GetAllocator());
    SendMessage(CMPCT, GET_HEADERS, head, from);
  }
  ...
  if (!requestBlocks.empty())
  {
    ...
    rapidjson::Value   array(rapidjson::kArrayType);
    int totalBlockMessageSize = 0;
    for (j=0;j<requestBlocks.size();j++) 
    {
        rapidjson::Value blockInfo(rapidjson::kObjectType);
    

        value = d["cmpct"][requestBlocks[j]]["height"].GetInt();
        blockInfo.AddMember("height", value, doc1.GetAllocator ());

        value = d["cmpct"][requestBlocks[j]]["minerId"].GetInt();
        blockInfo.AddMember("minerId", value, doc1.GetAllocator ());

        value = d["cmpct"][requestBlocks[j]]["parentBlockMinerId"].GetInt();
        blockInfo.AddMember("parentBlockMinerId", value, doc1.GetAllocator ());
        
        value = d["cmpct"][requestBlocks[j]]["numOfShortID"].GetInt()*2 + 34;
        totalBlockMessageSize += value.GetInt();
        blockInfo.AddMember("size", value, doc1.GetAllocator ());

        value = d["cmpct"][requestBlocks[j]]["timeCreated"].GetDouble();
        blockInfo.AddMember("timeCreated", value, doc1.GetAllocator ());

        value = d["cmpct"][requestBlocks[j]]["timeReceived"].GetDouble();             
        blockInfo.AddMember("timeReceived", value, doc1.GetAllocator ());

        value = d["cmpct"][requestBlocks[j]]["numOfShortID"].GetInt();
        blockInfo.AddMember("numOfShortID", value, doc1.GetAllocator());
        
        value = d["cmpct"][requestBlocks[j]]["numOfPrefilledTransactions"].GetInt();
        blockInfo.AddMember("numOfPrefilledTransactions", value, doc1.GetAllocator());

        value = d["cmpct"][requestBlocks[j]]["numOfTransactions"].GetInt();
        blockInfo.AddMember("numOfTransactions", value, doc1.GetAllocator());

        array.PushBack(blockInfo, doc1.GetAllocator());
    }   

    doc1.AddMember("cmpct", array, doc1.GetAllocator());
    value=CMPCT;
    doc1.AddMember("message", value, doc1.GetAllocator());
    ...
    rapidjson::StringBuffer packetGBT;
    rapidjson::Writer<rapidjson::StringBuffer> writerGBT(packetGBT);
    doc1.Accept(writerGBT);
    std::string packetGBTstr=packetGBT.GetString();

    m_sendBlockTimes.push_back(Simulator::Now().GetSeconds() + eventTime + sendTime);
    Simulator::Schedule(Seconds(eventTime), &BitcoinNode::SendGetBlockTxn, this, packetGBTstr, from );
    Simulator::Schedule(Seconds(eventTime + sendTime), &BitcoinNode::RemoveSendTime, this);      
  ...
  if(!receivedBlocks.empty())
  {
    ...
    for(j=0;j<receivedBlocks.size();j++)
    {
      rapidjson::Value blockObj(rapidjson::kObjectType);
      value = d["cmpct"][receivedBlocks[j]]["height"].GetInt();
      blockObj.AddMember("height", value, doc2.GetAllocator ());

      value = d["cmpct"][receivedBlocks[j]]["minerId"].GetInt();
      blockObj.AddMember("minerId", value, doc2.GetAllocator ());

      value = d["cmpct"][receivedBlocks[j]]["parentBlockMinerId"].GetInt();
      blockObj.AddMember("parentBlockMinerId", value, doc2.GetAllocator ());
      
      unsigned int blockSize;
      blockSize=(d["cmpct"][receivedBlocks[j]]["numOfShortID"].GetInt()+d["cmpct"][receivedBlocks[j]]["numOfPrefilledTransactions"].GetInt())*m_averageTransactionSize;
      value = blockSize;
      blockObj.AddMember("size", value, doc2.GetAllocator ());

      value = d["cmpct"][receivedBlocks[j]]["timeCreated"].GetDouble();
      blockObj.AddMember("timeCreated", value, doc2.GetAllocator ());

      value = d["cmpct"][receivedBlocks[j]]["timeReceived"].GetDouble();             
      blockObj.AddMember("timeReceived", value, doc2.GetAllocator ());
    
      array.PushBack(blockObj, doc2.GetAllocator());
    }
    doc2.AddMember("blocks", array, doc2.GetAllocator());

    rapidjson::StringBuffer blockbuff;
    rapidjson::Writer<rapidjson::StringBuffer> blockWriter(blockbuff);
    doc2.Accept(blockWriter);
    std::string help = blockbuff.GetString();
    ReceivedBlockMessage(help,from);
  }
  m_nodeStats->NumGBTSentInCmpct+=requestBlocks.size();
  m_nodeStats->NumFullBlockInCmpct+=receivedBlocks.size();
}
...
BitcoinNode::AdvertiseNewBlock (const Block &newBlock) 
{
  ...
  if(m_protocolType==COMPACT_BLOCK)
  {
    ...
    unsigned int blockSize = newBlock.GetBlockSizeBytes ();
    ...
    blockSize=92+numOfShortID*6+numOfPrefilledTransactions*m_averageTransactionSize;

    rapidjson::Value blockInfo(rapidjson::kObjectType);
    NS_LOG_INFO ("In advertise CMPCT " << newBlock);
    ...
    d.Accept(writer);
    int count=0;
    for(std::vector<Ipv4Address>::iterator it=m_sendHighBandwidth.begin();it!=m_sendHighBandwidth.end();it++)
    {
      count++;
      if ( *it != newBlock.GetReceivedFromIpv4 () )
      {
        const uint8_t delimiter[] = "#";
    ...
    m_nodeStats->CmpctSentBytes+=m_sendHighBandwidth.size()*blockSize;

    d.RemoveMember("cmpct");

    d["message"].SetInt(INV);

    stringStream << newBlock.GetBlockHeight () << "/" << newBlock.GetMinerId ();
    blockHash = stringStream.str();
    value.SetString(blockHash.c_str(), blockHash.size(), d.GetAllocator());
    array1.PushBack(value, d.GetAllocator());
    d.AddMember("inv", array1, d.GetAllocator());

    rapidjson::StringBuffer packetInfo1;
    rapidjson::Writer<rapidjson::StringBuffer> writer1(packetInfo1);
    d.Accept(writer1);
    int count1=0;
    for(std::vector<Ipv4Address>::iterator it=m_sendLowBandwidth.begin();it!=m_sendLowBandwidth.end();it++)
    {
      count1++;
      if ( *it != newBlock.GetReceivedFromIpv4 () )
      {
    ...
    m_nodeStats->invSentBytes+=(m_bitcoinMessageHeader + m_countBytes + d["inv"].Size()*m_inventorySizeBytes)*m_sendLowBandwidth.size();
...
BitcoinNode::SendMessage(enum Messages receivedMessage,  enum Messages responseMessage, rapidjson::Document &d, Ptr<Socket> outgoingSocket)
{
  ...
  switch (d["message"].GetInt()) 
  {
    ...
    case MSG_CMPCT_BLOCK:
    {
      m_nodeStats->MsgCmpctBlockSentBytes+= m_bitcoinMessageHeader + m_countBytes + d["blocks"].Size()*m_inventorySizeBytes;
      break;
    }
    case CMPCT:
    {
      for(int k = 0; k < d["cmpct"].Size(); k++)
        m_nodeStats->CmpctSentBytes += d["cmpct"][k]["size"].GetInt();
      break;
    }
    case GET_BLOCK_TXN:
    {
      for(int k = 0; k < d["cmpct"].Size(); k++)
        m_nodeStats->GetBlockTxnSentBytes += d["cmpct"][k]["size"].GetInt();
      break;
    }
    case BLOCK_TXN:
    {
      for(int k = 0; k < d["cmpct"].Size(); k++)
        m_nodeStats->BlockTxnSentBytes += d["cmpct"][k]["size"].GetInt();
      break;
    }
    case SENDCMPCT:
    {
      m_nodeStats->SendCmpctSentBytes+=9;
      std::cout<<"sendcmpctSentBytes= "<<m_nodeStats->SendCmpctSentBytes<<"\n";
    break;
    }
