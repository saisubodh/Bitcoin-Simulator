...
/*   double bitcoinMinersHash[] = {0.4, 0.3, 0.3};
  ...
  cmd.AddValue ("compactblock", "Change protocol to use compact blocks", compactblock);
  ...
  for(auto &miner : miners)
  {
	...
	if (systemId == targetNode->GetSystemId())
	{
      ...
      if (compactblock)    
        bitcoinMinerHelper.SetProtocolType(COMPACT_BLOCK);	  
      if (blockTorrent)	
      {		  
  ...
  for(auto &node : nodesConnections)
  {
	...
	if (systemId == targetNode->GetSystemId())
	{
      ...
      if ( std::find(miners.begin(), miners.end(), node.first) == miners.end() )
	  {
        ...
        if (compactblock)   
          bitcoinNodeHelper.SetProtocolType(COMPACT_BLOCK);	
        if (blockTorrent)	  
        {
...
/*         std::cout << "SystemId " << systemId << ": Node " << node.first << " with systemId = " << targetNode->GetSystemId() 
  ...
  MPI_Datatype   dtypes[51] = {MPI_INT, MPI_DOUBLE, MPI_DOUBLE, MPI_DOUBLE, MPI_INT, MPI_INT, MPI_INT, MPI_INT, MPI_DOUBLE, MPI_DOUBLE, MPI_DOUBLE, MPI_INT,
                               ...
                               MPI_LONG, MPI_LONG, MPI_LONG, MPI_LONG, MPI_LONG, MPI_LONG, MPI_LONG, MPI_LONG, MPI_INT, MPI_INT, MPI_INT, MPI_LONG, MPI_LONG, MPI_INT,MPI_LONG, MPI_LONG, MPI_LONG, MPI_LONG, MPI_LONG, MPI_LONG, MPI_LONG, MPI_LONG, MPI_LONG, MPI_LONG, MPI_LONG, MPI_LONG, MPI_LONG}; 
  ...
  disp[38] = offsetof(nodeStatistics, CmpctReceivedBytes);
  disp[39] = offsetof(nodeStatistics, BlockTxnReceivedBytes);
  disp[40] = offsetof(nodeStatistics, GetBlockTxnReceivedBytes);
  disp[41] = offsetof(nodeStatistics, MsgCmpctBlockReceivedBytes);
  disp[42] = offsetof(nodeStatistics, SendCmpctReceivedBytes);
  disp[43] = offsetof(nodeStatistics, CmpctSentBytes);
  disp[44] = offsetof(nodeStatistics, BlockTxnSentBytes);
  disp[45] = offsetof(nodeStatistics, GetBlockTxnSentBytes);
  disp[46] = offsetof(nodeStatistics, SendCmpctSentBytes);
  disp[47] = offsetof(nodeStatistics, MsgCmpctBlockSentBytes);
  disp[48] = offsetof(nodeStatistics, CmpctBlockNotProcessed);
  disp[49] = offsetof(nodeStatistics, NumGBTSentInCmpct);
  disp[50] = offsetof(nodeStatistics, NumFullBlockInCmpct);
  ...
  else if (systemId == 0 && systemCount > 1)
  {
    ...
    //            <<  " from systemId = " << status.MPI_SOURCE << "\n"; 
      ...
      stats[recv.nodeId].CmpctReceivedBytes = recv.CmpctReceivedBytes;
      ...
      stats[recv.nodeId].MsgCmpctBlockReceivedBytes = recv.MsgCmpctBlockReceivedBytes;
      stats[recv.nodeId].SendCmpctReceivedBytes = recv.SendCmpctReceivedBytes;
      stats[recv.nodeId].CmpctSentBytes = recv.CmpctSentBytes;
      ...
      stats[recv.nodeId].MsgCmpctBlockSentBytes = recv.MsgCmpctBlockSentBytes;
      stats[recv.nodeId].SendCmpctSentBytes = recv.SendCmpctSentBytes;
      stats[recv.nodeId].CmpctBlockNotProcessed = recv.CmpctBlockNotProcessed;
      stats[recv.nodeId].NumGBTSentInCmpct = recv.NumGBTSentInCmpct;
      stats[recv.nodeId].NumFullBlockInCmpct = recv.NumFullBlockInCmpct;
  ...
  if (systemId == 0)
  {
    ...
    PrintTotalStats(stats, totalNoNodes, tStartSimulation, tFinish, averageBlockGenIntervalMinutes, relayNetwork);
	   if(compactblock)
      std::cout<<"\nThe Protocol Type was COMPACT BLOCK\n";
     else if(sendheaders)
      std::cout<<"\nThe Protocol Type was SENDHEADERS\n";
    if(unsolicited)
...
void PrintStatsForEachNode (nodeStatistics *stats, int totalNodes)
{
  ...
  for (int it = 0; it < totalNodes; it++ )
  {
    ...
    std::cout << "The Compact Blocks received but not processed =  " << stats[it].CmpctBlockNotProcessed << " Bytes\n";
    double GBT=stats[it].NumGBTSentInCmpct, FB=stats[it].NumFullBlockInCmpct;
    double percent=GBT/(GBT+FB);

    std::cout << "The percentage of GETBLOCKTXN sent in response to CMPCT BLOCK =" << percent << " Bytes\n";
    ...
    std::cout << "The total received Compact Block messages were " << stats[it].CmpctReceivedBytes << " Bytes\n";
    ...
    std::cout << "The total received MSG_CMPCT_BLOCK messages were " << stats[it].MsgCmpctBlockReceivedBytes << " Bytes\n";
    std::cout << "The total received SENDCMPCT messages were " << stats[it].SendCmpctReceivedBytes << " Bytes\n";
    ...
    std::cout << "The total sent Compact Block messages were " << stats[it].CmpctSentBytes << " Bytes\n";
    ...
    std::cout << "The total sent MSG_CMPCT_BLOCK messages were " << stats[it].MsgCmpctBlockSentBytes << " Bytes\n";
    std::cout << "The total sent SENDCMPCT messages were " << stats[it].SendCmpctSentBytes << " Bytes\n";
...
void PrintTotalStats (nodeStatistics *stats, int totalNodes, double start, double finish, double averageBlockGenIntervalMinutes, bool relayNetwork)
{
  ...
  double     download = 0;
  double     upload = 0;
  double     CmpctReceivedBytes=0;
  double     BlockTxnReceivedBytes=0;
  double     GetBlockTxnReceivedBytes;
  double     MsgCmpctBlockReceivedBytes=0;
  double     SendCmpctReceivedBytes=0;
  double     CmpctSentBytes=0;
  double     BlockTxnSentBytes=0;
  double     GetBlockTxnSentBytes=0;
  double     MsgCmpctBlockSentBytes=0;
  double     SendCmpctSentBytes=0;
  double     GBTpercent=0;
  double     CmpctBlockNotProcessed;
  ...
  for (int it = 0; it < totalNodes; it++ )
  {
    ...
    CmpctReceivedBytes = CmpctReceivedBytes*it/static_cast<double>(it + 1) + stats[it].CmpctReceivedBytes/static_cast<double>(it + 1);
    ...
    MsgCmpctBlockReceivedBytes = MsgCmpctBlockReceivedBytes*it/static_cast<double>(it + 1) + stats[it].MsgCmpctBlockReceivedBytes/static_cast<double>(it + 1);
    SendCmpctReceivedBytes = SendCmpctReceivedBytes*it/static_cast<double>(it + 1) + stats[it].SendCmpctReceivedBytes/static_cast<double>(it + 1);
	
    CmpctSentBytes = CmpctSentBytes*it/static_cast<double>(it + 1) + stats[it].CmpctSentBytes/static_cast<double>(it + 1);
    ...
    MsgCmpctBlockSentBytes = MsgCmpctBlockSentBytes*it/static_cast<double>(it + 1) + stats[it].MsgCmpctBlockSentBytes/static_cast<double>(it + 1);
    SendCmpctSentBytes = SendCmpctSentBytes*it/static_cast<double>(it + 1) + stats[it].SendCmpctSentBytes/static_cast<double>(it + 1);

    CmpctBlockNotProcessed = CmpctBlockNotProcessed*it/static_cast<double>(it + 1) + stats[it].CmpctBlockNotProcessed/static_cast<double>(it + 1);
    
    double GBT=stats[it].NumGBTSentInCmpct, FB=stats[it].NumFullBlockInCmpct;
    double percent=GBT/(GBT+FB);
    GBTpercent = GBTpercent*it/static_cast<double>(it + 1) + percent/static_cast<double>(it + 1);
    ...
    download = stats[it].invReceivedBytes + stats[it].getHeadersReceivedBytes + stats[it].headersReceivedBytes
             ...
             + stats[it].extGetDataReceivedBytes + stats[it].chunkReceivedBytes + stats[it].CmpctReceivedBytes 
             + stats[it].BlockTxnReceivedBytes + stats[it].GetBlockTxnReceivedBytes + stats[it].MsgCmpctBlockReceivedBytes 
             + stats[it].SendCmpctReceivedBytes;
    ...
    upload = stats[it].invSentBytes + stats[it].getHeadersSentBytes + stats[it].headersSentBytes
           ...
           + stats[it].extGetDataSentBytes + stats[it].chunkSentBytes + stats[it].CmpctSentBytes 
           + stats[it].BlockTxnSentBytes + stats[it].GetBlockTxnSentBytes + stats[it].MsgCmpctBlockSentBytes 
           + stats[it].SendCmpctSentBytes;
  ...
  averageBandwidthPerNode = invReceivedBytes + invSentBytes + getHeadersReceivedBytes + getHeadersSentBytes + headersReceivedBytes
                          ...
                          + extHeadersSentBytes + extGetDataReceivedBytes + extGetDataSentBytes + chunkReceivedBytes + chunkSentBytes + CmpctReceivedBytes 
                          + BlockTxnReceivedBytes + GetBlockTxnReceivedBytes + MsgCmpctBlockReceivedBytes 
                          + SendCmpctReceivedBytes + CmpctSentBytes + BlockTxnSentBytes + GetBlockTxnSentBytes 
                          + MsgCmpctBlockSentBytes + SendCmpctSentBytes;
...
std::cout << "The average received Compact Blocks that were not processed = "<<CmpctBlockNotProcessed<<"Bytes\n";
std::cout << "The percentage of times GETBLOCKTXN was sent in response to CMPCT BLOCK message(when processed) = "<<GBTpercent*100<<"%\n\n";
  ...
  std::cout << "\nThe average received Compact Block messages were " << CmpctReceivedBytes << " Bytes (" 
            << 100. * CmpctReceivedBytes / averageBandwidthPerNode << "%)\n";
  ...
  std::cout << "The average received MSG_CMPCT_BLOCK messages were " << MsgCmpctBlockReceivedBytes << " Bytes (" 
            << 100. * MsgCmpctBlockReceivedBytes / averageBandwidthPerNode << "%)\n";
  std::cout << "The average received SENDCMPCT messages were " << SendCmpctReceivedBytes << " Bytes (" 
            << 100. * SendCmpctReceivedBytes / averageBandwidthPerNode << "%)\n";
  

  std::cout << "The average sent Compact Block messages were " << CmpctSentBytes << " Bytes (" 
            << 100. * CmpctSentBytes / averageBandwidthPerNode << "%)\n";
  ...
  std::cout << "The average sent MSG_CMPCT_BLOCK messages were " << MsgCmpctBlockSentBytes << " Bytes (" 
            << 100. * MsgCmpctBlockSentBytes / averageBandwidthPerNode << "%)\n";
  std::cout << "The average sent SENDCMPCT messages were " << SendCmpctSentBytes << " Bytes (" 
            << 100. * SendCmpctSentBytes / averageBandwidthPerNode << "%)\n\n";
  ...
  std::cout << "Total average traffic due to Compact Block messages = " << CmpctReceivedBytes +  CmpctSentBytes << " Bytes(" 
            << 100. * (CmpctReceivedBytes +  CmpctSentBytes) / averageBandwidthPerNode << "%)\n";
  ...
  std::cout << "Total average traffic due to MSG_CMPCT_BLOCK messages = " << MsgCmpctBlockReceivedBytes +  MsgCmpctBlockSentBytes << " Bytes(" 
            << 100. * (MsgCmpctBlockReceivedBytes +  MsgCmpctBlockSentBytes) / averageBandwidthPerNode << "%)\n";
  std::cout << "Total average traffic due to SENDCMPCT messages = " << SendCmpctReceivedBytes +  SendCmpctSentBytes << " Bytes(" 
            << 100. * (SendCmpctReceivedBytes +  SendCmpctSentBytes) / averageBandwidthPerNode << "%)\n";
