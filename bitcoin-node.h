...
protected:
  ...
  void SendCmpctBlock(std::string packetInfo, Address& from);

  void  ReceivedCmpctBlockMessage(std::string &blockInfo, Address &from);

  void SendGetBlockTxn(std::string &msgInfo, Address &from);

  void CalculateCmpctBlock(std::string &blockHash);

  void setCompact(int maxHighBandwidthRelay);

  void MCBSendBlock(std::string &packet, Address &from);
  
  void SendBlockTxn(std::string blockInfo, Address &from);

  void GBTSendBlock(std::string blockInfo, Address &from);

  void ReceivedGBTMessage(std::string &blockInfo, Address &from);

  void ReceivedBTMessage(std::string &blockInfo, Address &from);
  ...
  Time 	m_cmptblocktimeout;

  std::vector<std::string>      m_queueCmpct;
  std::vector<Ipv4Address>      m_sendHighBandwidth;
  std::vector<Ipv4Address>      m_sendLowBandwidth;
  std::vector<Ipv4Address>      m_receiveHighBandwidth;
  std::vector<Ipv4Address>      m_receiveLowBandwidth;
  unsigned int                  m_queueCmpctSize;
  std::map<std::string,bool>    m_receivedCmpctBlock;
  unsigned int                  m_CmpctBufferSize=1000;
  const double                  m_maxHighBandwidthRelay=1000;
  const double                  m_CmpctEstimateShortIdTransactions=95;
  const double                  m_CmpctEstimatedTransactionsNotPresent=10;
